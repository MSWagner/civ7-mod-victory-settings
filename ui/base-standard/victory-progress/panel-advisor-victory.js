...
class PanelAdvisorVictory extends Panel {
    ...
    onAttach() {
        ...
        for (const setupParam of GameSetup.getGameParameters()) {
            ...
            if (!setupParam.hidden && setupParam.invalidReason == GameSetupParameterInvalidReason.Valid) {
                ...
                const isVictory = VICTORY_PARAMETERS.some(param => param == parameterID);
                if (!isVictory) {
                    continue;
                }
                ...
                if (value === "true") {
                    switch (parameterID) {
                      case "MilitaryVictoryEnabled":
                          this.EnabledVictoryAdvisorTypes.push("ADVISOR_MILITARY");
                          break;
                      case "ScienceVictoryEnabled":
                          this.EnabledVictoryAdvisorTypes.push("ADVISOR_SCIENCE");
                          break;
                      case "EconomicVictoryEnabled":
                          this.EnabledVictoryAdvisorTypes.push("ADVISOR_ECONOMIC");
                          break;
                      case "CultureVictoryEnabled":
                          this.EnabledVictoryAdvisorTypes.push("ADVISOR_CULTURE");
                          break;
                    }
                }
            }
    ...
    createQuestCarousel() {
        ...
        this.carouselScrollable = carouselContentContainer;
        // text box
        ...
        bodyText.classList.add('body-text', 'font-body', 'text-sm', 'text-center', 'px-4', 'py-1', 'flex-auto', 'min-w-128');
        ...
        radioWrapper.classList.add('flow-row', 'mt-6', 'absolute', '-bottom-8');
    ...
    */
    ...
    renderRewardPip(milestoneRewards, ageProgressRewardAmt) {
        ...
        for (let i = 0; i < milestoneRewards.length; i++) {
            ...
            const tipDescription = Locale.compose(milestoneRewards[i].Description || '');
    ...
    renderBottomPanel(chosenAdvisor) {
        ...
        legendReward.classList.add('font-fit-shrink', 'w-36', 'min-h-24', 'break-words');
        ...
        for (const reward of finalReward) {
            ...
            const tipDescription = Locale.compose(reward.Description || '');
    ...
    renderTrackVictoryCheckBox() {
        ...
        const isTrackerVisible = this.activeQuest ? true : false;
    ...
    handleEngineInput(inputEvent) {
        if (inputEvent.detail.name == 'scroll-pan') {
            this.carouselScrollable?.dispatchEvent(InputEngineEvent.CreateNewEvent(inputEvent));
            return false;
        }
